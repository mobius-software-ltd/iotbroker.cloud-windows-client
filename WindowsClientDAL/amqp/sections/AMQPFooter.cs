...
{
    public class AMQPFooter : AMQPSection
    {
        #region private fields

        private Dictionary<AMQPSymbol, Object> _annotations;
        ...
        public TLVAmqp getValue()
        {
            ...
            if (_annotations != null)
                map = AMQPWrapper<AMQPSymbol>.wrapMap(_annotations);

            DescribedConstructor constructor = new DescribedConstructor(map.Code, new TLVFixed(AMQPType.SMALL_ULONG, new byte[] { 0x78 }));
        ...
        {
            if (!map.isNull())
                _annotations = AMQPUnwrapper<AMQPSymbol>.unwrapMap(map);
        }
        ...
        public Dictionary<AMQPSymbol, Object> Annotations
        {
            get
            {
                return _annotations;
        ...
        public void addAnnotations(String key, Object value)
        {
            if (key == null)
                throw new ArgumentException("Annotations map cannot contain objects with null keys");

            if (_annotations == null)
                _annotations = new Dictionary<AMQPSymbol, Object>();

            _annotations[new AMQPSymbol(key)] = value;
        }
